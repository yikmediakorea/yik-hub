...
async function ttLookup(handle, rapidKey) {
  const username = handle.replace("@", "").trim();
  const r = await fetch(`https://tiktok-scraper7.p.rapidapi.com/user/info?unique_id=${username}`, {
    headers: { "x-rapidapi-key": rapidKey, "x-rapidapi-host": "tiktok-scraper7.p.rapidapi.com" }
  });
  const d = await r.json();
  if (!r.ok) throw new Error(d.msg || "TikTok 조회 실패");
  // handle both response structures
  const userInfo = d.data?.userInfo || d.data || d;
  const u = userInfo?.user || userInfo;
  const s = userInfo?.stats || d.data?.stats || {};
  if (!u?.uniqueId && !u?.nickname) throw new Error("TikTok 유저를 찾을 수 없습니다");
  return {
    id: uid(), platform: "TikTok", channelId: u?.id,
    name: u?.nickname, handle: `@${u?.uniqueId || username}`,
    followers: s?.followerCount || u?.followerCount || 0,
    views: s?.videoCount || u?.videoCount || 0,
    posts: s?.videoCount || u?.videoCount || 0,
    country: "",
    thumb: u?.avatarMedium || u?.avatarThumb, description: u?.signature,
...
function SocialTab({ rapidKey, rapidOk, addToList, list }) {
  ...
  const addManual = () => {
    if (!manual.name && !manual.handle) return;
    addToList({
    ...
    <div>
      ...
      {platform === "TikTok" && (
        <div>
          {!rapidOk && (
            <Notice c="warn">
              TikTok 조회에는 RapidAPI 키가 필요합니다.<br />
              1. <a href="https://rapidapi.com" target="_blank" rel="noopener noreferrer">rapidapi.com</a> 로그인<br />
              2. <b>Tiktok Scraper</b> 검색 → 구독 → 키를 API 설정에 입력
            </Notice>
          )}
          <div style={S.card}>
            ...
            {mode === "single" ? (
              <div style={{ display: "flex", gap: 8 }}>
                <input value={handle} onChange={e => setHandle(e.target.value)} onKeyDown={e => e.key === "Enter" && searchSingle()}
          ...
          {!loading && results.length === 0 && !err && (
            <Empty>@username을 입력하고 조회하세요.</Empty>
          )}
        </div>
      )}
      ...
      {platform === "Instagram" && (
        <div>
          <Notice c="info">
            Instagram은 공식 API 제한으로 자동 조회가 불안정합니다.<br />
            프로필에서 직접 확인한 수치를 입력하시면 리스트에 바로 저장됩니다.
          </Notice>
          <div style={S.card}>
            <div style={{ fontSize: 13, fontWeight: 700, marginBottom: 14 }}>Instagram 인플루언서 추가</div>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "0 16px" }}>
              <Fld label="핸들 *"><input value={manual.handle} onChange={e => setManual(p => ({...p, handle: e.target.value}))} placeholder="@username" style={S.input} /></Fld>
              <Fld label="이름"><input value={manual.name} onChange={e => setManual(p => ({...p, name: e.target.value}))} placeholder="홍길동 / 채널명" style={S.input} /></Fld>
            </div>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: "0 16px" }}>
              <Fld label="팔로워 수"><input value={manual.followers} onChange={e => setManual(p => ({...p, followers: e.target.value}))} placeholder="150000" style={S.input} /></Fld>
              <Fld label="게시물 수"><input value={manual.posts} onChange={e => setManual(p => ({...p, posts: e.target.value}))} placeholder="320" style={S.input} /></Fld>
              <Fld label="국가"><input value={manual.country} onChange={e => setManual(p => ({...p, country: e.target.value}))} placeholder="KR / US / ID" style={S.input} /></Fld>
            </div>
            <Fld label="채널 특성 / 메모">
              <input value={manual.description} onChange={e => setManual(p => ({...p, description: e.target.value}))} placeholder="뷰티/라이프스타일, MZ 타깃, 브랜디드 광고 경험 있음" style={S.input} />
            </Fld>
            <div style={{ marginTop: 8, display: "flex", alignItems: "center", gap: 12 }}>
              <button style={S.btn("p")} onClick={addManual} disabled={!manual.handle}>+ 리스트에 추가</button>
              {manual.followers && <span style={{ fontSize: 12, color: "#888" }}>예상 단가: {estimatePrice("Instagram", manual.followers.replace(/[^0-9]/g,""))}</span>}
            </div>
          </div>
          <Empty>인스타그램 프로필에서 수치 확인 후 입력하세요.<br />저장된 데이터는 리스트 탭에서 엑셀로 내보낼 수 있습니다.</Empty>
        </div>
      )}
    </div>
  );
